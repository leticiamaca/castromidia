...
const SERVICES = [
  {
    cor:   'text-gold', bg: 'bg-gold/10', bgHover: 'group-hover:bg-gold/20',
    titulo: 'Google Ads',
  ...
  {
    cor:   'text-rose', bg: 'bg-rose/10', bgHover: 'group-hover:bg-rose/20',
    titulo: 'Meta Ads',
  ...
  },
  {
    ...
    desc:   'Estratégia completa de captação, qualificação e nutrição de leads para o seu funil de vendas.',
    ...
    <section id="servicos" className="py-28 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-14">
          <p className="font-body text-xs tracking-widest uppercase text-gold mb-2">O que fazemos</p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          {SERVICES.map(({ cor, bg, bgHover, titulo, desc, icon }) => (
            <div key={titulo} className="card rounded-2xl p-6 group">
              <div className={`w-11 h-11 rounded-xl ${bg} ${bgHover} transition-colors flex items-center justify-center mb-5`}>
                <svg className={`w-5 h-5 ${cor}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  {icon}
                </svg>
              </div>
              <h3 className="font-display text-lg font-semibold text-gray-100 mb-2">{titulo}</h3>
          ...
          <div className="rounded-2xl p-6 border border-gold/25 bg-gradient-to-br from-gold/10 via-transparent to-rose/8 flex flex-col justify-between">
            <div>
              <h3 className="font-display text-lg font-semibold text-gray-100 mb-2">Pronto para crescer?</h3>
              <p className="font-body text-sm text-gray-400 leading-relaxed">
                Diagnóstico gratuito das suas campanhas atuais — veja onde estão as oportunidades perdidas.
              </p>
            </div>
            <a
